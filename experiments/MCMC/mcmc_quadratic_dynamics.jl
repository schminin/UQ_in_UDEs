...
@model function bayes_nn(data, ude_prob)
    ...
    par_α ~ LogUniform(0.1, 10.0) # transformed
    par_np ~ Uniform(-10, 10) # transformed
    par_nn ~ MvNormal(zeros(nparameters), sig .* ones(nparameters))
    
    parameters = merge((par_α = par_α,), (nn=vector_to_parameters(par_nn, p_init.nn),))

    predicted = solve(ude_prob; p=parameters, saveat=t_obs)

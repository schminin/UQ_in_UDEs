...
experiment_name = ARGS[3] # experiment_name = "ground_truth_gaussian_IR_noise_0.01"
...
# predefine st
nn_model, p_init, st = prepare_model(hyperpar, 1, sample_mechanistic, noise_model, random_init)
...
models_per_cpu = Int(aleatoric_noise_realizations*ensemble_members/n_cpus)
for (data_id, model_id) in collect(Iterators.product(1:aleatoric_noise_realizations, 1:ensemble_members))[1+models_per_cpu*(array_nr-1):models_per_cpu*array_nr]
    try
        # println(model_id)

...
function load_and_evaluate_model(experiment_path, data_id, model_id, hyperpar)
    ...
    global nn_model = prepare_model(hyperpar, model_id, sample_mechanistic, noise_model, random_init)
    metrics = []
    metric_names = []
    data_plot = []
    par_list = [(p_opt, "opt")]
    for par in par_list
        ...
        if size(data.pred_train)==size(data.X_train)  # n_timepoints t_train == n_obs X_train
            ...
            if problem_name in ["seir", "seir_pulse"]
                β_pred = repeat([mech_par.β_opt], length(data.t_plot))
    ...
    col_names = ["data_id", "model_id", metric_names..., labels(mech_par)...]
    col_entry = [data_id, model_id, metrics..., getdata(mech_par)...]

...
experiment_path = joinpath(@__DIR__, "..", "experiments", "MCMC", "pigeon", problem_name, experiment_name)
...
pred_agg = CSV.read(joinpath(experiment_path, "aggregate_predictions.csv"), DataFrame)
...
constant_parameters_summary = let
    ...
    dens1 = CairoMakie.density!(ax1, retrieve_α.(vcat(Array(get(mcmc_res, :par_α).par_α)...)), normalization = :pdf, 
    ...
    dens2 = CairoMakie.density!(ax2, retrieve_γ.(vcat(Array(get(mcmc_res, :par_γ).par_γ)...)), normalization = :pdf, 
    ...
    if noise_model=="Gaussian"
        dens3 = CairoMakie.density!(ax3, sqrt.(exp.(vcat(Array(get(mcmc_res, :par_np).par_np)...))), normalization = :pdf, 
                    label_size = 18, strokecolor = (:white, 0.5), color=(:lightblue)) 
                    vline = vlines!(ax3, [noise_par]; color=:tomato4, linewidth=3)
    elseif noise_model=="negBin"
        dens3 = CairoMakie.density!(ax3, 1 .+ exp.((vcat(Array(get(mcmc_res, :par_np).par_np)...))), normalization = :pdf, 
                    label_size = 18, strokecolor = (:white, 0.5), color=(:lightblue)) 
                    vline = vlines!(ax3, [noise_par]; color=:tomato4, linewidth=3)
    end
...
save(joinpath("paper_figures", "plots", problem_name, experiment_name, "MCMC_constant_parameters.pdf"), constant_parameters_summary)
...
save(joinpath("paper_figures", "plots", problem_name, experiment_name, "MCMC_time_varying_parameter_overview.pdf"), time_varying_parameter_summary)
